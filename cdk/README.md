...
- Define a stack instance in `cdk/stacks/warclaude-stack.ts`:
...
interface WarclaudeStackProps extends cdk.StackProps {
}
export class WarclaudeStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: WarclaudeStackProps) {
...
import { WarclaudeStack } from "./warclaude-stack";

export const makeStacks = (app: App) => {
  const stacks = [
    new WarclaudeStack(app, "WarclaudeStack", {
